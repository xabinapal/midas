...
<svelte:head><title>{data.isDraft ? "Editar borrador" : "Editar gasto previsto"} | {site.title}</title></svelte:head>

<div class="mx-auto max-w-md space-y-6">
	<header>
		<h1 class="text-2xl font-bold tracking-tight">{data.isDraft ? "Editar borrador" : "Editar gasto previsto"}</h1>
		<p class="text-sm text-[var(--color-text-soft)]">
			Solo los gastos previstos sin pagos se editan así; los importes reales se corrigen con reversión.
		</p>
	</header>
		...
		<div class="card-body gap-4">
			<form method="POST" use:enhance class="flex flex-col gap-4">
				...
				<label class="fieldset" for="amount">
					<span class="fieldset-legend">Importe previsto ({data.currency})</span>
					<input
						id="amount"
						name="amount"
						type="text"
				...
				<details class="collapse collapse-arrow rounded-box border border-[var(--color-border)]">
					...
					<div class="collapse-content flex flex-col gap-4">
						<label class="fieldset" for="dueDate">
							...
							<input id="dueDate" name="dueDate" type="date" class="input min-h-12 w-full" bind:value={$form.dueDate} />
							{#if $errors.dueDate}
