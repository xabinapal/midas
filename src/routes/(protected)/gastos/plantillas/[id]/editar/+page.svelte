<script lang="ts">
	...
	const initialValues = untrack(() =>
		Object.fromEntries(
			data.form.data.memberIds.map((memberId, index) => [memberId, data.form.data.memberValues[index] ?? ""]),
		),
	);
</script>

<svelte:head><title>Editar plantilla | {site.title}</title></svelte:head>

<div class="mx-auto max-w-md space-y-6">
	<header>
		<h1 class="text-2xl font-bold tracking-tight">Editar plantilla</h1>
		<p class="text-sm text-[var(--color-text-soft)]">{data.template.description}</p>
	</header>
