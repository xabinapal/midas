...
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **C6**  | Operation gate is dead code — never imported by any route                                                                                  | **Open** | The `activity-history` spec requires replay-safe multi-record operations with gate acquisition, operation roots, and idempotent retry. The gate service exists but no route uses it. All mutations are direct DB writes with `operation_id: null`. This is a significant spec divergence. Wiring it requires refactoring every mutating route action. |
| **W9**  | Unit-tested credential functions (`changeOwnPassword`, `listSessions`, `revokeUserSession`) are dead code; routes reimplement logic inline | **Open** | The `credentials.ts` functions are tested but routes don't import them. The inline route code is untested.                                                                                                                                                                                                                                            |
| **W11** | Bootstrap doesn't write an operation root for replay safety                                                                                | **Open** | Spec says "An expired holder SHALL be recovered from its bootstrap operation root." `performBootstrap` only resets the gate to `available` on failure.                                                                                                                                                                                                |
| **S1**  | `defaultWeight` input missing from member create form                                                                                      | **Open** | Schema defines it, form doesn't render it. Members always get weight 0.                                                                                                                                                                                                                                                                               |
| **S3**  | Member management not admin-gated                                                                                                          | **Open** | Spec is ambiguous; any authenticated user can create/deactivate members. May be intentional.                                                                                                                                                                                                                                                          |
| **S4**  | Recovery schema doesn't normalize username                                                                                                 | **Open** | Route lowercases manually but schema lacks `.transform()`.                                                                                                                                                                                                                                                                                            |
