...
Defines household expense recognition as an event independent from payment and settlement: categories with an active lifecycle, exact multi-member allocations resolved deterministically to minor units, stable human-readable references, payments as singular account outflows with explicit funding sources, many-to-many payment applications, derived payment and due states, safe evidence references, and correction by linked reversal and replacement.
