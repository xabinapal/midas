...
### Make multi-record operations replay-safe without assuming transactions

D1 through the current Kysely dialect does not provide the transaction contract needed for these workflows. Multi-record commands therefore use a stable operation ID and a pending/complete operation root. A single-row conditional update on the household command gate admits at most one in-flight mutating operation for that household, including competing operation IDs from different isolates. The gate records operation ID, expected household version, and lease expiry. Losing admission returns a conflict/retry result; an expired gate is recovered from its operation root before another command is admitted.

Child records and the allow-listed activity event reference the admitted operation. Mutable concepts such as credentials, user state, and administrator designation use immutable versions selected only from completed operations, so pending writes do not replace the current value. Retrying the same ID and safe payload fingerprint completes missing writes or returns the completed result; reusing an ID with a different payload is rejected. Sensitive commands stage only protected domain values such as the supported password hash, never plaintext or a reusable plaintext fingerprint. Only the original actor or the recovery service may resume an operation. A failed pending operation is recoverable and never appears as posted household state.

Operations whose result contains a newly generated bearer token are an explicit secret-result exception. The token remains transient and only its digest is stored. Retrying a completed secret-result operation returns a generic `fresh authentication required` outcome rather than the token; the client starts a new login operation. A lost rotation response may require login again. Persisting bearer tokens for replay was rejected because replay convenience does not justify expanding credential exposure.

Claiming ordinary service-level writes were atomic was rejected because a Worker can fail between statements. Per-subject optimistic checks without a household gate were rejected because one command can span expense, payment, account, session, and activity aggregates. Raw D1 transaction calls were rejected by the repository persistence boundary. Household-level serialization is acceptable for the expected low write volume and keeps invariant admission explicit.
